...
export default class ExamCreatePage extends React.Component {
  ...
  render() {
    ...
    return (
      <Page loading={!!loading} header={header} mainClassName={styles['exam-create-page-main']}>
        ...
        <div className={styles["steps-action"]}>
          {
            current > 0 &&
            <Button size="large" style={{marginRight: 30}} onClick={this.prev}>上一步</Button>
          }
          {
            current < stepLimitNum &&
            <Button type="primary" size="large" style={{marginRight: 30}} onClick={this.onNext}>下一步</Button>
          }
          {
            current === stepLimitNum &&
