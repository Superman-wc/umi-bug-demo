...
@Form.create()
export default function CreateFilePanel(props) {

  const {form: {getFieldDecorator, validateFieldsAndScroll}, onSubmit} = props;
          ...
          <Fragment key={index}>
            ...
            }
            {
              Object.entries(items).map(([key, {fieldOptions, children, ...itemProps}]) =>
                <Form.Item {...itemProps}>
                  {
                    getFieldDecorator(key, fieldOptions)(children)
                  }
                </Form.Item>
              )
            }
